...
const DashboardKaryawan = () => {
  ...
  return (
    ...
    <div className="flex flex-row w-full min-h-screen  bg-[#f5f5f5]">

      <div className="relative h-full">
        <aside className='fixed left-0 top-0 bg-white text-black h-screen w-64 flex flex-col space-y-2 gap-2 p-4'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              {routes.map((item, index) => (
                <Link href={item.path} key={`item-${item.name}-${index}`}
                  className={`w-full flex flex-row gap-2 items-center py-3 pl-2 rounded-xl font-semibold hover:bg-[#f5f5f5] hover:text-black
        ...
        <main className="w-full pl-80 pt-20">
          <h2 className="text-2xl font-semibold text-black">Login as Admin</h2>
        </main>
